	...
	static parse(text, keepbooltype := false, as_map := true) {
		...
		Loop Parse text, '"' {
			...
			if !Q {
				...
				if (t := Trim(A_LoopField, " `t`r`n")) = "," || (t = ":" && V := 1)
					...
					Loop Parse t {
						...
						if InStr("`n`r `t", A_LoopField)
							...
							C := A_LoopField = "[" ? [] : maptype()
							A ? D[L].Push(C) : map_set(D[L], K, C)
							D.Has(++L) ? D[L] := C : D.Push(C)
							V := ""
							A := Type(C) = "Array"
							continue
						} else if InStr("]}", A_LoopField) {
				...
				OutputDebug	A_LineNumber "`tNQ → Json Class"
				continue
			}
			else if A	{
				OutputDebug	A_LineNumber "`tA → JSON Class"
				LF := P A_LoopField, C.Push(InStr(LF, "\") ? UC(LF) : LF), P := ""
			}
			else if V	{	;	Value
				LF := P A_LoopField, map_set(C, K, InStr(LF, "\") ? UC(LF) : LF), K := V := P := ""
				; OutputDebug	A_LineNumber "`tValue: " LF
			}
			else	{		;	Key
				LF := P A_LoopField, K := InStr(LF, "\") ? UC(LF) : LF, P := ""
				; OutputDebug	A_LineNumber "`tKey: " LF
			}
		}
		return J
