...
vetor := '70FC01AA8FCA3900E384EA28A5B7BCEF'
comando := '{ "oper":[{"acao":"ident", "a":"139565", "nome":"Kah", "id":1234}] }'
z_functions := 0
...
start_server() {
	global sock := winsock("server", cb, "IPV4")
	; sock.Bind("0.0.0.0", 12345)
	sock.Bind("10.0.20.43",12345)
	sock.Listen()
...
z(string, key, vetor) {
	; string := StrReplace(string, "id_da_central", 0531)
	MsgBox Encrypt.String("AES", "CBC", string, key, vetor)
	OutputDebug string
...
class Encrypt extends CNG {
	...
	static String(AlgId, Mode, String, Key, IV := "", Encoding := "UTF-8", Output := "Base64") {
		...
		try
		{
			...
			CIPHER_LENGTH := this.BCrypt.Encrypt(hKey, DataBuf, DataBuf.Size - 1, IVBuf, IVSize, &CIPHER_DATA, this.BCrypt.Constants
	...
	static File(AlgId, Mode, FileNameIn, FileNameOut, Key, IV := "", Bytes := 1048576, Offset := 0, Length := -1, Encoding :=
		"UTF-8") {
		static hAlgorithm := 0, hKey := 0, IVBuf := 0, IVSize := 0
...
class Decrypt extends CNG {
	...
	static String(AlgId, Mode, String, Key, IV := "", Encoding := "UTF-8", Input := "Base64") {
		...
		try
		{
			...
			DECRYPT_LENGTH := this.BCrypt.Decrypt(hKey, CIPHER_DATA, CIPHER_LENGTH, IVBuf, IVSize, &DECRYPT_DATA, this.BCrypt.Constants
				.BCRYPT_BLOCK_PADDING)
